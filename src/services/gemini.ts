...
export async function askInvestigatorCopilot(
  ...
  if (res.ok) {
    ...
    // If server returned an HTML page (Netlify static SPA rewrite returning index.html for /api/copilot)
    if (contentType.includes('text/html')) {
      console.warn('Copilot received HTML response from server endpoint (static host deployment or Vite dev server). Invoking local grounded fallback.');
      return generateLocalGroundedCopilotResponse(
        request,
        precomputedConnection,
        'Running on static host deployment. This response was generated by deterministic case-data rules only.'
      );
    }
    ...
    if (rawText.trim().startsWith('<')) {
      console.warn('Copilot received HTML payload from dev server endpoint. Invoking local grounded fallback.');
      return generateLocalGroundedCopilotResponse(
        request,
        precomputedConnection,
        'Running on static host deployment. This response was generated by deterministic case-data rules only.'
  ...
  // 2. Explicit 503 / 502 / 504 / 404 / 405: Upstream API service unavailable or static host unconfigured -> Fallback is allowed
  if (res.status === 503 || res.status === 502 || res.status === 504 || res.status === 404 || res.status === 405) {
    console.warn(`Copilot service endpoint unavailable (HTTP ${res.status}), invoking local grounded fallback.`);
    return generateLocalGroundedCopilotResponse(
      request, 
      precomputedConnection,
      'API endpoint unavailable on static host. This response was generated by deterministic case-data rules only.'
    );
  }
